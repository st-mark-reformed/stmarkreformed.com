...
export default async function CalendarYearMonthPage (
    ...
    let { pagePath } = pageData;

    const pagePathArray = pagePath.split('/');

    pagePathArray[0] = 'calendar-test';
    ...
    if (prevPageData) {
        prevMonthLink = `/calendar-test/${prevPageParams.year}/${prevPageParams.month}`;
    }

    let nextMonthLink = '';

    if (nextPageData) {
        nextMonthLink = `/calendar-test/${nextPageParams.year}/${nextPageParams.month}`;
    }
            ...
            <div className="p-4 pb-10 max-w-1800px mx-auto">
                <div className="lg:flex lg:h-full lg:flex-col">
                    <CalendarPageHeader
                        ...
                        icsUrl={`${getConfigString(ConfigOptions.BASE_URL)}/calendar-test/ics`}
