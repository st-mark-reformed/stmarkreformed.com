...
export default function MenOfTheMarkSearchForm (
    {
        defaultKeyword,
    }: {
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (trimmed === '') {
            router.push('/admin/men-of-the-mark');

            return;
        }

        router.push(`/admin/men-of-the-mark?keyword=${encodeURIComponent(trimmed)}`);
    };
    ...
    return (
        <form onSubmit={submit} className="flex w-full items-center gap-2 sm:w-auto">
            <input
                type="search"
                name="keyword"
                aria-label="Search Men of the Mark"
                placeholder="Search Men of the Mark…"
                value={keyword}
