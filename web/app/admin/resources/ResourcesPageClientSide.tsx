...
import SubmitDeleteResourcesFormAction from './SubmitDeleteResourcesFormAction';
import Alert from '../../Alert';
import Pagination from '../../Pagination/Pagination';
import ResourcesSearchForm from './ResourcesSearchForm';

export default function ResourcesPageClientSide (
    {
        resourceItems,
        currentPage,
        totalPages,
    ...
    const [state, formAction, isPending] = useActionState(
        ...
        SubmitDeleteResourcesFormAction,
        {
            status: 'unsubmitted',
            iteration: 0,
            message: '',
    ...
    return (
        <>
            <Breadcrumbs />
            <PageTitle buttons={buttons}>Resources</PageTitle>
            {(() => {
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>{pagination}</div>
                <div className="pb-3">
                    <ResourcesSearchForm defaultKeyword={keyword} />
                </div>
            </div>
            <CardList
                ref={formRef}
                formAction={formAction}
                noItemsFoundMessage="No resources found."
                items={resourceItems.map((resourceItem) => ({
                    id: resourceItem.id,
                    columns: [
                        {
                            id: `title-${resourceItem.id}`,
                            line1: <>{resourceItem.title} <span className="text-xs font-light ml-2">(slug: {resourceItem.slug})</span></>,
                            line2: resourceItem.isEnabled ? 'Enabled' : 'Disabled',
                        },
                        {
                            id: `metadata-${resourceItem.id}`,
                            line1: resourceItem.date,
                            line2: `${resourceItem.downloads.length} download${resourceItem.downloads.length === 1 ? '' : 's'}`,
                        },
                        [
                            {
                                content: 'View Page',
                            ...
                            {
                                content: 'Edit',
                                href: `/admin/resources/edit/${resourceItem.id}`,
                                type: 'primary',
                                rightGlyph: 'pencil',
                            },
                        ],
