...
export default function HymnsOfTheMonthSearchForm (
    {
        defaultKeyword,
    }: {
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (trimmed === '') {
            router.push('/admin/hymns-of-the-month');

            return;
        }

        router.push(`/admin/hymns-of-the-month?keyword=${encodeURIComponent(trimmed)}`);
    };
    ...
    return (
        <form onSubmit={submit} className="flex w-full items-center gap-2 sm:w-auto">
            <input
                type="search"
                name="keyword"
                aria-label="Search Hymns of the Month"
                placeholder="Search Hymns of the Month…"
                value={keyword}
