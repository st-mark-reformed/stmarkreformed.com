...
export default function PastorsPageSearchForm (
    {
        defaultKeyword,
    }: {
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (trimmed === '') {
            router.push('/admin/pastors-page');

            return;
        }

        router.push(`/admin/pastors-page?keyword=${encodeURIComponent(trimmed)}`);
    };
    ...
    return (
        <form onSubmit={submit} className="flex w-full items-center gap-2 sm:w-auto">
            <input
                type="search"
                name="keyword"
                aria-label="Search Pastor's Page"
                placeholder="Search Pastor's Page…"
                value={keyword}
