<?php

declare(strict_types=1);

namespace App\Http\PageBuilder\BlockResponse\LatestGalleries;

use App\Http\PageBuilder\BlockResponse\BlockResponseBuilderContract;
use App\Http\PageBuilder\BlockResponse\LatestGalleries\Entities\LatestGalleriesContentModel;
use craft\elements\MatrixBlock;
use craft\errors\InvalidFieldException;
use Twig\Environment as TwigEnvironment;
use Twig\Error\LoaderError;
use Twig\Error\RuntimeError;
use Twig\Error\SyntaxError;
use yii\base\InvalidConfigException;

class LatestGalleries implements BlockResponseBuilderContract
{
    public function __construct(
        private TwigEnvironment $twig,
        private LatestGalleriesRetriever $latestGalleriesRetriever,
    ) {
    }

    /**
     * @throws LoaderError
     * @throws RuntimeError
     * @throws SyntaxError
     * @throws InvalidFieldException
     * @throws InvalidConfigException
     *
     * @phpstan-ignore-next-line
     */
    public function buildResponse(MatrixBlock $matrixBlock): string
    {
        return $this->twig->render(
            '@app/Http/PageBuilder/BlockResponse/LatestGalleries/LatestGalleries.twig',
            [
                'contentModel' => new LatestGalleriesContentModel(
                    heading: (string) $matrixBlock->getFieldValue(
                        'heading',
                    ),
                    subHeading: (string) $matrixBlock->getFieldValue(
                        'subHeading',
                    ),
                    galleryItems: $this->latestGalleriesRetriever->retrieve(),
                ),
            ],
        );
    }
}
