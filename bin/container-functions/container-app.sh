...
function container-app() {
    ...
    if [[ -z "${allArgsExceptFirst}" ]]; then
        ...
        docker exec ${interactiveArgs} -e XDEBUG_MODE=off -w /opt/project stmark-app bash;
    else
        docker exec ${interactiveArgs} -w /opt/project stmark-app bash -c "XDEBUG_MODE=off ${allArgsExceptFirst}";
    fi
