...
readonly class ResourceItemFactory
{
    public function __construct(private ResourceUploadResolver $uploadResolver)
    {
    }

    public function createFromRequest(ServerRequest $request): ResourceItem
    {
        try {
            $id = Uuid::fromString(
                $request->attributes->getString(name: 'resourceId'),
            );
        } catch (Throwable) {
            $id = new EmptyUuid();
        }
