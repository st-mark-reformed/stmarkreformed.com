...
readonly class PastorsPageItemFactory
{
    public function createFromRequest(ServerRequest $request): PastorsPageItem
    {
        try {
            $id = Uuid::fromString(
                $request->attributes->getString(name: 'pastorsPageId'),
            );
        } catch (Throwable) {
            $id = new EmptyUuid();
        }
